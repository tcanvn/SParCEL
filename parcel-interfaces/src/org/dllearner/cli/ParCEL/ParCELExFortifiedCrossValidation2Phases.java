...
/**
 * Cross validation procedure with fortification for ParCEL-Ex
 * 
...
public class ParCELExFortifiedCrossValidation2Phases extends CrossValidation {
	...
	*/
	...
	public ParCELExFortifiedCrossValidation2Phases(AbstractCELA la, PosNegLP lp, AbstractReasonerComponent rs,
	 ...
	 */
	public ParCELExFortifiedCrossValidation2Phases(AbstractCELA la, ParCELPosNegLP lp, AbstractReasonerComponent rs,
			int folds, boolean leaveOneOut, int noOfRuns) {
		...
		for (int kk=0; kk < noOfRuns; kk++) {
			...
			for(int currFold=0; (currFold<folds); currFold++) {
				...
				long learningMili1 = ((ParCELExAbstract)la).getLearningTime();
				...
				// learning time, does not include the reduction time
				long learningMili2 = ((ParCELExAbstract)la).getLearningTime();
				...
				outputWriter("Learning finished. Concept length: " + concept.getLength() + ", number of pdef used: " + parcelEx.getNoOfReducedPartialDefinition());
				...
				//cpdef: some stat information (this should be revised as we find the cpdefs in another phase
				int noOfUsedCpdef = parcelEx.getNumberOfCounterPartialDefinitionUsed();
				int noOfCpdef = counterPartialDefinitions.size();
				noOfCpdefStat.addNumber(noOfCpdef);				
				this.noOfCpdefUsedStat.addNumber(noOfUsedCpdef);
