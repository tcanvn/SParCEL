...
//	private EvaluatedDescriptionSet evaluatedDescriptions = new EvaluatedDescriptionSet(LearningAlgorithm.MAX_NR_OF_RESULTS);
	...
	private LongWrapper totalRefinement;
	private long countTotalRefinement=0;
...
//			OCELConfigurator configurator,
			...
			LongWrapper totalDescriptions) {
	
			this.totalRefinement = totalDescriptions;
...
//				manchester += show + ": " + c.toManchesterSyntaxString(baseURI, prefixes) + "\n";
		...
		//return total refinememt, this is not check if the total refinement = total descriptions generated
		totalRefinement.setValue(this.countTotalRefinement);
	}
...
//		System.out.println("max length: " + maxLength);
		...
		// this is much faster , but you get another candidate
		// set(??)
		refinements.removeAll(node.getChildConcepts());
		
		//TODO: calculate the number of descriptions here?
		//countTotalRefinement++;
		countTotalRefinement += refinements.size();
