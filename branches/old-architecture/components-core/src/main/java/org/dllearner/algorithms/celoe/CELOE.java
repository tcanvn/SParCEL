 ...
 * @author An C. Tran
 * 
 * This class is modified by @author An C. Tran so that it can:
 * <ol>
 * 	<li>employ and return "partial definitions"</li>
 * 	<li>terminate when partial definitions cover all positive examples (this use for fortification)
 * </ol>
 *
 */
...
public class CELOE extends AbstractCELA implements CELOEMBean {
...
//	private OEHeuristicRuntime heuristicRuntime = new OEHeuristicRuntime();
	...
	private double noise;		//will be assign = noisePercentage/100
	...
	private int posSize, negSize;		//AnTran: these vars are added for learning partial definitions 
	...
	@ConfigOption(name = "maxNoOfSplits", defaultValue="10", description="Max no of splits used for the refinement operator")
...
//		singleSuggestionMode = configurator.getSingleSuggestionMode();
		...
		*/
		...
		((RhoDRDown)operator).setObjectPropertyHierarchy(reasoner.getObjectPropertyHierarchy());
		((RhoDRDown)operator).setDataPropertyHierarchy(reasoner.getDatatypePropertyHierarchy());
		((RhoDRDown)operator).setUseStringDatatypes(true);
		((RhoDRDown)operator).setUseHasValueConstructor(this.useHasValue);
...
//					System.out.println(reasoner.getIndividuals(existingDefinition));
	...
	public double getCurrentlyBestAccuracy() {
		if ((bestEvaluatedDescriptions != null) && (bestEvaluatedDescriptions.getBest() != null))
...
//		System.out.println(configurator.getMaxExecutionTimeInSeconds());
		...
		try {
			ObjectName workerPoolName = new ObjectName(
					"org.dllearner.algorithms.ParCEL.ParCELearnerMBean:type=PDLLReducerMBean");
		...
		partialDefinitions =  new HashSet<PartialDefinition>();
		unCoveredPos = new HashSet<Individual>();
		...
		unCoveredPos.addAll(((PosNegLPStandard)learningProblem).getPositiveExamples());
		
		allPosCovered = (unCoveredPos.size() == 0);
		
		//----------------------
//		System.out.println("Noise: " + noise);
//		System.out.println("Positive examples: " + posSize +
//				"; Negative examples: " + negSize);		
//		System.out.println("Min coverage allowed: " + (posSize - Math.ceil(posSize * noise)));
...
//		System.out.println("isRunning: " + isRunning);
	...
	private TreeSet<Description> refineNode(OENode node) {
		...
		//System.out.println("refinement result: " + refinements);
...
//		System.out.println("Test " + new Date());
		...
		if(accuracy > 2 || (accuracy < 0.0 && accuracy != -1)) {
		...
		if (accuracy > 1) {
			...
			HashSet<Individual> coveredPosTmp = new HashSet<Individual>();
			for (Individual ind : unCoveredPos) {
...
//			System.out.println("minimum horizontal expansion is now " + minHorizExp);
	...
	public int getCurrentlyBestDescriptionLength() {
		return getCurrentlyBestDescription().getLength();
	}
	
	public long getRunningTime() {
		return (System.currentTimeMillis() - nanoStartTime/1000);
	}
	
	
	public Set<PartialDefinition> getPartialDefinitions() {
