...
public class ParCELCorrectnessGreedyReducer {
			...
			Set<Individual> negativeExamples) 
	{
		...
		positiveExamplesTmp.addAll(negativeExamples);
		...
		synchronized (counterPartialDefinitions) {
			...
			for (int i = 0; (positiveExamplesTmp.size() > 0)
				...
				int counti = 0;
				for (Individual indi : ((ParCELExtraNode) partialDefs[i]).getCoveredPositiveExamples()) {
				...
				for (int j = i + 1; j < counterPartialDefinitions.size(); j++) {
					...
					for (Individual indj : ((ParCELExtraNode) partialDefs[j]).getCoveredPositiveExamples())
